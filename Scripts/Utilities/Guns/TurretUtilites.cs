...
namespace Benito.ScriptingFoundations.Utilities
{
    ...
    public static class TurretUtilities
    {
            ...
            Transform turretAnchorTransform, Transform gunAnchorTransform, TurretSettings settings, float deltaTime)
        {
            ...
            float targetTurretAngles = Vector2.SignedAngle(targetDirectionInTurretSpace.ToVector2_xz(), Vector2.up);
            ...
            float targetGunAngles = Vector2.SignedAngle(targetGunSpaceDirectionFlattened.normalized, Vector2.right);

            // allow to go over back with turret
            if (!settings.hasTurretRotLimit)
            {
                if (state.turretRotation < 0 && targetTurretAngles > 0 && targetTurretAngles - state.turretRotation > 180)
                {
                    targetTurretAngles = targetTurretAngles - 360;
                }
                else if(state.turretRotation > 0 && targetTurretAngles < 0 && targetTurretAngles - state.turretRotation < -180)
                {
                    targetTurretAngles = targetTurretAngles + 360;
                }
            }
            ...
            state.turretRotation = FloatUtilities.BlendLinearly(state.turretRotation, targetTurretAngles, settings.turretRotationSpeed, deltaTime);
            state.gunRotation = FloatUtilities.BlendLinearly(state.gunRotation, targetGunAngles, settings.gunRotationSpeed, deltaTime);
            ...
            Transform turretAnchorTransform, Transform gunAnchorTransform, TurretSettings settings, float deltaTime)
        {
            ...
            state.turretRotation = FloatUtilities.BlendWithAccelAndDecel(state.turretRotation, targetTurretAngles, 
                settings.turretRotationSpeed, settings.turretAcceleration,ref state.turretVelocity, true, deltaTime);
            state.gunRotation = FloatUtilities.BlendWithAccelAndDecel(state.gunRotation, targetGunAngles, 
